...
/**
 * 모의 법정 역할 Function Calling Tools
 *
 * Phase 2: AI가 재판 과정에서 각 역할의 함수를 호출
 */
@Component
@Slf4j
public class TrialRoleTools {
     ...
     * 판사 - 개정 선언
     */
    @Description("판사가 법정을 개정하고 재판을 시작합니다")
    public Function<OpenTrialRequest, OpenTrialResponse> judgeOpenTrial() {
        return request -> {
     ...
     * 검사 - 기소 및 구형
     */
    @Description("검사가 피고인을 기소하고 형량을 구형합니다")
    public Function<ProsecuteRequest, ProsecuteResponse> prosecutorProsecute() {
        return request -> {
     ...
     * 변호사 - 변론 및 감형 요청
     */
    @Description("변호사가 피고인을 변론하고 정상참작을 요청합니다")
    public Function<DefendRequest, DefendResponse> attorneyDefend() {
        return request -> {
     ...
     * 판사 - 최종 판결
     */
    @Description("판사가 최종 판결을 선고합니다")
    public Function<VerdictRequest, VerdictResponse> judgeVerdict() {
        return request -> {
